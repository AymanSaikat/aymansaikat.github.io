...
Cache purged. Direct data terminal link is active. What would you like to request?`,
  ...
  const renderMessageContent = (text: string) => {
    ...
    return blocks.map((block, bIdx) => {
      if (block.type === "code") {
        ...
        let lang = "";
        let codeBody = block.content;
        if (codeLines[0] && codeLines[0].length < 15 && !codeLines[0].includes(" ") && !codeLines[0].includes("{")) {
          lang = codeLines[0];
          codeBody = codeLines.slice(1).join("\n");
        }
        return (
          <div key={bIdx} className="my-3 font-mono text-[0.68rem] bg-[#0d0d14]/90 border border-white/10 rounded-[2px] overflow-hidden shadow-inner">
            <div className="bg-white/[0.04] px-3 py-1.5 border-b border-white/[0.06] flex justify-between items-center text-[0.55rem] text-[#8a8a93] uppercase tracking-widest font-bold">
              <span className="flex items-center gap-1.5">
                <span className="inline-block w-1.5 h-1.5 rounded-full bg-gold animate-pulse" />
                {lang || "SYSTEM COMPILER"}
              </span>
              <button 
                onClick={() => {
                  navigator.clipboard.writeText(codeBody.trim());
                }}
                className="hover:text-gold active:scale-95 transition-all interactive-cursor font-mono cursor-pointer"
              >
                Copy Code
              </button>
            </div>
            <pre className="p-3.5 overflow-x-auto text-gold-light leading-relaxed whitespace-pre font-mono scrollbar-thin">
      ...
      return lines.map((line, idx) => {
        ...
        // Handle Headers
        if (line.startsWith("##### ")) {
          return <h5 key={`${bIdx}-${idx}`} className="font-mono text-xs font-bold text-gold tracking-wider mt-3 mb-1 uppercase flex items-center gap-1.5">{line.replace("##### ", "")}</h5>;
        }
        if (line.startsWith("#### ")) {
          return <h4 key={`${bIdx}-${idx}`} className="font-mono text-[0.68rem] font-black text-gold tracking-widest mt-4 mb-1 uppercase">{line.replace("#### ", "")}</h4>;
        }
        if (line.startsWith("### ")) {
          return <h3 key={`${bIdx}-${idx}`} className="font-sans text-xs font-semibold text-text-primary mt-4 mb-1">{line.replace("### ", "")}</h3>;
        }
        ...
        if (line.trim().startsWith("- ") || line.trim().startsWith("* ")) {
          ...
          content = (
            <span className="flex items-start gap-1.5 pl-1 my-1">
              <span className="text-gold shrink-0 mt-1.5 text-[0.4rem]">■</span>
              <span className="flex-1">{formatInlineMarkdown(itemText)}</span>
            </span>
          );
          return <div key={`${bIdx}-${idx}`} className="my-0.5">{content}</div>;
        }

        if (!line.trim()) return <div key={`${bIdx}-${idx}`} className="h-2" />;

        return (
          <p key={`${bIdx}-${idx}`} className="my-1.5 leading-relaxed">
  ...
  const formatInlineMarkdown = (text: string) => {
    ...
    while ((match = boldRegex.exec(text)) !== null) {
      ...
      parts.push(
        <strong key={match.index} className="text-gold font-bold">
          {match[1]}
        </strong>
      );
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* Chat conversations workspace area with smooth animations */}
            <div className="flex-1 overflow-y-auto p-4 space-y-4 font-sans text-xs text-muted-lavender bg-gradient-to-b from-bg-dark/20 to-bg-panel/50 scrollbar-thin">
                ...
                {messages.map((msg) => (
                  ...
                  >
                    ...
                    <div className={`p-3 rounded-[3px] border relative ${
                      msg.role === "user"
                        ? "bg-gold/5 border-gold/20 text-text-primary text-left"
            ...
            {/* Dynamic Suggestion Prompt Overlay inside viewport - Animated nicely */}
            {messages.length < 3 && (
              <div className="px-4 py-3 bg-bg-dark/45 border-t border-white/[0.03] space-y-2">
                  ...
                  {suggestions.map((s, idx) => (
                    ...
                    >
                      <button
                        ...
                        className="interactive-cursor w-full flex items-center gap-1.5 text-left font-mono text-[0.55rem] text-gold/85 hover:text-gold hover:bg-gold/5 border border-gold/10 hover:border-gold/35 px-2.5 py-1.5 rounded-[1px] transition-all whitespace-normal cursor-pointer active:scale-[0.99]"
                      >
